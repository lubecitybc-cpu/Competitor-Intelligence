"""Run the Mobil 1 Lube Express v2 scraper (Calgary + Edmonton).

Reads ``app/config/competitors.v2.json`` and only runs Mobil 1 Lube Express.
Writes:
    data/promotions/mobil1_lube_express_v2.json
    data/promotions/mobil1_lube_express_v2.csv
    data/promotions/mobil1_lube_express_v2_url_coverage.csv
    data/promotions/mobil1_lube_express_v2_excluded_rows.csv  (only if exclusions)

Usage:
    python run_mobil1_lube_express_v2.py                  # default --qa-expanded
    python run_mobil1_lube_express_v2.py --final-deduped  # one row per (group, city)
    python run_mobil1_lube_express_v2.py --no-ocr         # skip image OCR
"""
...
from app.scrapers.mobil1_lube_express_scraper import (  # noqa: E402
    scrape_mobil1_lube_express_v2,
)
...
def find_mobil1(config_path: Path) -> dict:
    data = json.loads(config_path.read_text())
    for entry in data:
        name = entry.get("competitor", "").strip().lower()
        if name == "mobil 1 lube express":
            return entry
    raise SystemExit("Mobil 1 Lube Express not found in competitors.v2.json")
...
def export_url_coverage(result: dict, dest: Path, competitor: str) -> Path:
    ...
    fields = [
        "competitor", "url", "scope", "page_kind", "service_hint", "status",
        "cards_on_page",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run Mobil 1 Lube Express v2 scraper",
    )
    ...
    g.add_argument("--final-deduped", dest="mode", action="store_const",
                   const="final_deduped",
                   help="One row per (duplicate_group_id, city).")
    parser.add_argument("--no-ocr", action="store_true",
                        help="Skip coupon image OCR.")
    ...
    if not cfg.exists():
        ...
        return 1
    entry = find_mobil1(cfg)

    print("=" * 70)
    print("Mobil 1 Lube Express v2")
    print("=" * 70)
    ...
    t0 = time.time()
    result = scrape_mobil1_lube_express_v2(
        entry, mode=args.mode, enable_ocr=not args.no_ocr,
    )
    ...
    if url_log:
        ...
        for e in url_log:
            print(
                f"  [{e.get('page_kind','?'):<14}|{e.get('status','?'):<13}] "
    ...
    if promos:
        print(f"Sample rows (first 12 of {len(promos)}):")
        for i, p in enumerate(promos[:12], 1):
            ...
            print(
                ...
                f"d={p.get('discount_value')!r} c={p.get('coupon_code')!r}{nr}"
            )
            print(f"     title: {str(p.get('ad_title',''))[:100]}")
            print(f"     desc : {str(p.get('promo_description',''))[:100]}")
            print(f"     cities: {p.get('applicable_cities')}")
        print()

    json_path = ROOT / "data" / "promotions" / "mobil1_lube_express_v2.json"
    csv_path = ROOT / "data" / "promotions" / "mobil1_lube_express_v2.csv"
    coverage_path = ROOT / "data" / "promotions" / "mobil1_lube_express_v2_url_coverage.csv"
    excluded_path = ROOT / "data" / "promotions" / "mobil1_lube_express_v2_excluded_rows.csv"
