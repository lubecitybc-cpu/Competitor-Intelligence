"""Run the LubeFx Plus v2 scraper (Edmonton, city_store).

Reads ``app/config/competitors.v2.json``, runs only the ``LubeFx Plus`` entry,
and writes:
    data/promotions/lubefx_v2.json
    data/promotions/lubefx_v2.csv
    data/promotions/lubefx_v2_url_coverage.csv
    data/promotions/lubefx_v2_excluded_rows.csv     (only if exclusions exist)

Usage:
    python run_lubefx_v2.py                  # default --qa-expanded
    python run_lubefx_v2.py --final-deduped  # one row per duplicate_group_id
    python run_lubefx_v2.py --no-ocr         # skip coupon image OCR
"""
...
from app.scrapers.lubefx_scraper import scrape_lubefx_v2  # noqa: E402


# Sheet-compatible columns FIRST, then QA / meta columns (per spec).
CSV_PREFERRED = [
...
def find_lubefx(config_path: Path) -> dict:
    data = json.loads(config_path.read_text())
    for entry in data:
        if entry.get("competitor", "").strip().lower() == "lubefx plus":
            return entry
    raise SystemExit("LubeFx Plus not found in competitors.v2.json")


def export_promotions_to_csv(rows: list, dest: Path) -> Path:
    """Write rows to CSV with the spec column order, appending any extras."""
    if not rows:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run LubeFx Plus v2 scraper")
    ...
    if not cfg.exists():
        ...
        return 1
    entry = find_lubefx(cfg)

    print("=" * 70)
    print("LubeFx Plus v2")
    print("=" * 70)
    ...
    t0 = time.time()
    result = scrape_lubefx_v2(entry, mode=args.mode, enable_ocr=not args.no_ocr)
    ...
    if url_log:
        ...
        for e in url_log:
            print(
                f"  [{e.get('page_kind','?'):<8}|{e.get('status','?'):<13}] "
    ...
    if promos:
        print(f"Sample rows (first 8 of {len(promos)}):")
        for i, p in enumerate(promos[:8], 1):
    ...
    json_path = ROOT / "data" / "promotions" / "lubefx_v2.json"
    csv_path = ROOT / "data" / "promotions" / "lubefx_v2.csv"
    coverage_path = ROOT / "data" / "promotions" / "lubefx_v2_url_coverage.csv"
    excluded_path = ROOT / "data" / "promotions" / "lubefx_v2_excluded_rows.csv"
