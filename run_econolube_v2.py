"""Run the Econo Lube v2 scraper (Edmonton, city_store).

Reads ``app/config/competitors.v2.json`` and only runs Econo Lube. Writes:
    data/promotions/econolube_v2.json
    data/promotions/econolube_v2.csv
    data/promotions/econolube_v2_url_coverage.csv
    data/promotions/econolube_v2_excluded_rows.csv  (only if exclusions)

Usage:
    python run_econolube_v2.py                  # default --qa-expanded
    python run_econolube_v2.py --final-deduped  # one row per duplicate_group_id
    python run_econolube_v2.py --ocr            # enable OCR for homepage images
"""
...
from app.scrapers.econolube_scraper import scrape_econolube_v2  # noqa: E402
...
def find_econolube(config_path: Path) -> dict:
    data = json.loads(config_path.read_text())
    for entry in data:
        if entry.get("competitor", "").strip().lower() == "econo lube":
            return entry
    raise SystemExit("Econo Lube not found in competitors.v2.json")
...
def export_url_coverage(result: dict, dest: Path, competitor: str) -> Path:
    ...
    fields = [
        "competitor", "url", "scope", "service_hint", "is_homepage", "status",
        "cards_on_page",
    ...
    with dest.open("w", newline="", encoding="utf-8") as f:
        ...
        for e in val.get("url_log") or []:
            ...
            w.writerow({
                ...
                "is_homepage": e.get("is_homepage", False),
                "status": e.get("status", ""),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run Econo Lube v2 scraper")
    ...
    parser.add_argument("--ocr", action="store_true",
                        help="Enable image OCR on the homepage (off by default).")
    ...
    print(f"OCR        : {'ON (homepage only)' if args.ocr else 'OFF'}")
    print()

    t0 = time.time()
    result = scrape_econolube_v2(entry, mode=args.mode, enable_ocr=args.ocr)
    ...
    if url_log:
        ...
        for e in url_log:
            hp = "HP" if e.get("is_homepage") else "  "
            print(
                f"  [{hp}|{e.get('status','?'):<13}] "
    ...
    json_path = ROOT / "data" / "promotions" / "econolube_v2.json"
    csv_path = ROOT / "data" / "promotions" / "econolube_v2.csv"
    coverage_path = ROOT / "data" / "promotions" / "econolube_v2_url_coverage.csv"
    excluded_path = ROOT / "data" / "promotions" / "econolube_v2_excluded_rows.csv"
